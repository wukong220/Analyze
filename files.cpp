...
{
	os << "FileName  FileLabel" << endl;
	for (int i = 0; i < file.m_fnamebel.size(); i++)
	{
		for (int j = 0; j < file.m_fnamebel[i].size(); j++)
			os << file.m_fnamebel[i][j] << " ";
		os << endl;
	}
	os << endl;
	
	for(int i = 0; i < file.m_frames.size(); i++)
	{
		for (int j = 0; j < file.m_frames[i].size(); j++)
			os << file.m_frames << " ";
		os << endl;
	}
	return os;
