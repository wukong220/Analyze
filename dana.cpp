...
int frames = 2000;						//tau = steps_frame * frames
...
vector<int> msd = {0, 1, nAtoms/2, nAtoms};	//{0,1,nAtoms/2,nAtoms}
vector<int> pCom= {0,1,0,1};
...
int main() 
{
	...
	if( outdataA.size() && outdataA.size() != pCom.size() ){
		cout << "\"ERROR\": outdataA is wrongly set !  -> dana.cpp:24" << endl;
		output << "\"ERROR\": outdataA is wrongly set !  -> dana.cpp:24" << endl;
		exit(1);
	}
	else if( ! ( outdataB.size() || outdataA.size() ) ){
		cout << "\"ERROR\": outdata is NULL  -> dana.cpp:24, 25" << endl;
		output << "\"ERROR\": outdataA is wrongly set !  -> dana.cpp:24" << endl;
		exit(1);
	}
	...
	for (int ifile = 0; ifile < Num_files; ifile++)
	{
		...
		if(!fin.is_open()) 
		{
			cout << "\"ERROR\": Cannot open " << finname[ifile] << "--> dana.cpp:22";
			output << "\"ERROR\": outdataA is wrongly set !  -> dana.cpp:24" << endl;
			exit(1);
		}
		...
		for (int iframe = 1; iframe <= Num_frames; iframe++)//for md_dt
		{
			...
			{
				string error = "\"ERROR\": Timestep != iframe * steps_frame * cut[0] (dana.cpp:16) -> ";
...
//				v_cm[i] /= nAtoms;
		...
		for (int k = outdataA.size() ; k < outdata.size(); k++)
		{
			if(outdataB.size() && msd.size()){
				...
				output << "\ntime(dt = " << dstep * md_dt << ")\t\t" ;
				for (int p = 0; p < msd.size(); p++) {
					cout <<  "[F" << setw(3) << setfill('0') << Num_files << "][P" << setw(3) << setfill('0') << msd[p] << "]\t";
					output << "[F" << setw(3) << setfill('0') << Num_files << "][P" << setw(3) << setfill('0') << msd[p] << "]\t";
				}
				cout << endl;
				output << endl;
			}
